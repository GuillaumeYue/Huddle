...
- **Phase 5 core done, verified live:** `GooglePlacesProvider` (Places API New `searchNearby`, strict FieldMask, `includedPrimaryTypes` — includedTypes seated the Fairmont hotel at the table), pure mapping in `placesMapping.ts`, ~1km geo-grid Redis cache 6h TTL, provider chosen by `GOOGLE_PLACES_API_KEY` presence (mock fallback), dotenv loads `backend/.env`. Key restricted + $5 budget alert. Photos SHIPPED: FieldMask + per-place media lookup with skipHttpRedirect (keyless public googleusercontent url + author attribution into metadata — no wire change, invariant 1's dividend), cache key bumped to places:v2; iOS CardView layers AsyncImage over the mesh (gradient = placeholder/fallback), tiny attribution caption (ToS requirement). Photo SKU ≈ $0.14 per cell fill per TTL. Remaining polish (later): host-supplied room location (center via env for now).
...
- **Phase 7 done (history + deletion + deploy readiness).** Session history: `GET /users/:id/history` + iOS HistoryView from Home. **Account deletion SHIPPED, semantics decided: anonymize, never cascade** — name → 'Deleted user', apple_sub null, Redis taste profile purged; rooms/swipes/picks stay (group history is the group's). SIWA later only swaps the identity source. Deploy readiness: production `backend/Dockerfile` (server CMD, worker via command override), `APIConfig` Debug=localhost / Release=deploy-origin placeholder (FILL BEFORE FIRST TESTFLIGHT). **Write-behind lab retired as moot, honestly:** durable facts were synchronous to Postgres from day one, Redis held only rebuildables — no deferred-persistence gap exists to demonstrate.
- **Remaining before ship (phase 8, mostly console/account work):** dev account → SIWA → real-device testing → deploy backend (Railway/Fly, migrations, Release URLs, wss) → privacy policy → App Store Connect (screenshots, privacy labels) → TestFlight → review. Optional: 6.3 learn-from-outcomes, card photos.
...
- **Blind pick v2 SHIPPED (decision C):** `picks` table (005; one hidden pick per (room, round, user), PK-final), `resolvePick()` records + broadcasts `hasPicked` (the fact, never the card) and fires plurality resolution when the roster completes — same all-done + CAS shape as trigger A; timeout/sweeper resolve with whatever picks exist. Wire: `participants[].hasPicked`, `result.pickedBy`. iOS: cast-and-wait UI with local-only highlight. Original decision record: replace first-tap-wins with one round of simultaneous hidden picks — every roster member blind-picks one face-down tied card; when all have picked (or the pick timeout fires), the reveal shows the vote spread and the most-picked card wins, exact top ties broken by server random. Terminal by construction. Reuses existing machinery (per-user idempotent pick votes, all-done trigger, timeout rescue, CAS resolution). Rejected: B (two-stage blind-then-open — the blind stage resolves ~4% of the time with 3 cards × 4 people, two new states for theater) and keeping A (one person decides dinner). Current first-tap-wins stays until then.
