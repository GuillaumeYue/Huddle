...
- **Phase 4 remaining:** trigger B = inactivity timeout (→ distributed-timer lab: only one node may fire; SET NX lease on the timer), overtime rounds (round bump + fresh/relaxed deck + re-frozen roster, hard cap), user-tapped blind pick UI, tally/vote summary on the reveal screen.
